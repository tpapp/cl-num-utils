...
(addtest (ix-tests)
  hashed-index-tests
  (let* ((keys #(alpha (alpha 1) (beta) (kappa 9)))
         (index (make-hashed-index keys)))
    (ensure-same (ix-start index) 0)
    (ensure-same (ix-end index) (length keys))
    (ensure-same (ix index 'alpha) 0)
    (ensure-same (ix index '(alpha 1)) 1)
    (ensure-same (ix index '(beta)) 2)
    (ensure-same (ix index '(kappa 9)) 3)
    (ensure-error (ix index 'beta))
